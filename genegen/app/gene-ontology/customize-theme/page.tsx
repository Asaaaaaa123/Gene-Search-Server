...
export default function CustomizeTheme() {
  ...
  /** False until we have applied server or local preferences (avoids autosave wiping storage). */
  const [prefsHydrated, setPrefsHydrated] = useState(false);
  /** Where authoritative data came from / last successful server sync. */
  const [prefsSync, setPrefsSync] = useState<'idle' | 'cloud'>('idle');

  const applyCustomThemePayload = useCallback((ct: StoredCustomThemePreferences | null | undefined) => {
    if (!ct) return;
    if (ct.customThemes && Array.isArray(ct.customThemes)) {
      setCustomThemes(ct.customThemes as ThemeOption[]);
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const r = await fetchWithClerk(getToken, '/api/user/preferences');
        if (!cancelled && r.ok) {
          const d = await r.json();
          const ct = d?.customTheme as StoredCustomThemePreferences | undefined;
          applyCustomThemePayload(ct);
          if (ct) {
      ...
      if (!cancelled && !loadedFromApi) {
        const local = readStoredCustomTheme(userId);
        applyCustomThemePayload(local ?? undefined);
        setPrefsSync('idle');
      }
  ...
  }, [isLoaded, isSignedIn, userId, getToken, applyCustomThemePayload]);

  useEffect(() => {
    if (!isSignedIn || !userId || !prefsHydrated) return;
      ...
      (async () => {
        try {
          const r = await fetchWithClerk(getToken, '/api/user/preferences', {
            ...
            body: JSON.stringify({ customTheme: payload }),
          });
          if (r.ok) setPrefsSync('cloud');
        } catch {
          /* server sync failed; localStorage still updated above */
        }
      })();
    }, 1200);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div>
            ...
            {isLoaded && !isSignedIn && (
              <p className="mt-3 text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                Sign in to save your theme selections and custom themes across visits.{' '}
                <Link href="/sign-in" className="underline font-medium">
                  Sign in
                </Link>
              </p>
            )}
            {isSignedIn && prefsHydrated && prefsSync === 'cloud' && (
              <p className="mt-3 text-sm text-green-800 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                Your custom theme settings are saved to your account (server sync).
              </p>
            )}
          </div>
          <Link 
        ...
        {/* Theme overlap uses the same enrichment path as analysis; no need to run Analyze first */}
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h3 className="text-xl font-semibold text-gray-900 mb-2">Theme-Theme Gene Overlap Network</h3>
          <p className="text-gray-600 mb-4 text-sm">
            Nodes are themes; edges show how many genes are shared between themes (from GO term–gene intersections).
            Select a gene file and themes above, then generate — you can use this before or after &quot;Analyze Genes&quot;.
          </p>
          <button
            type="button"
            onClick={generateOverlapNetwork}
            disabled={!selectedFile || selectedThemes.length === 0 || overlapNetworkLoading}
